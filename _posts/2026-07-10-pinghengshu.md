...
# 平衡树专题（三）：AVL 树 —— 严格的平衡卫士

在前两篇中，我们领略了 Splay 的自适应均摊之美和 Treap 的随机化轻量之巧。它们均不以“每时每刻保持严格平衡”为目标，而是通过均摊或期望来保证总体效率。然而，在**实时系统、数据库索引、航空电子**等对单次操作延迟有严格上限的场景下，我们需要一种 **确定性（Deterministic）** 的平衡树，保证任何单次操作的最坏时间复杂度均为 $O(\log n)$。**AVL 树**正是这类数据结构中的先驱与典范。

AVL 树由 Georgy Adelson-Velsky 和 Evgenii Landis 于 1962 年发明，是历史上第一棵自平衡二叉搜索树。它通过引入 **平衡因子（Balance Factor）** 这一简单指标，并辅以有限的旋转操作，将树高严格控制在 $1.44 \log n$ 以内，从而提供了铁一般的性能承诺。
