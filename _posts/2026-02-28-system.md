...
*   在CMD中依次输入以下命令（每输一行按一次回车）：
    ...
    clean                  # 清空磁盘，删除所有现有分区
    convert gpt            # 将磁盘转换为GPT分区表（适用于现代UEFI启动的电脑如果你的电脑是老旧的传统BIOS，请使用 convert mbr）
    ```
2.  **创建分区**：
*   紧接上步，继续输入：
    ```bash
...
2.  **查看镜像包含的系统版本**：一个镜像文件（install.wim）里可能包含多个版本（如家庭版、专业版），输入以下命令查看：
    ...
    ```
    记下你想要安装的版本对应的**索引号**（Index），例如专业版通常是 `1`
3.  **应用镜像到系统盘**：
    ```bash
